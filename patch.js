...
// 2. Limit 40 button
const limit_btns = `<button id="btn-limit-30" class="tog-btn px-2 sm:px-3 py-1 rounded-md text-[11px] sm:text-xs font-bold transition-all bg-blue-600 text-white" onclick="setLimit(30)">30</button>
...
// 3. Header Buttons
const header_buttons = `<!-- Fullscreen -->
...
// 4. IDs and Dice buttons
for (let i = 1; i <= 3; i++) {
    content = content.replace(`onclick="faltaEnvido(${i})" class="quick-btn`, `id="btn-falta-envido-${i}" onclick="faltaEnvido(${i})" class="quick-btn`);
    const input_str = `onkeydown="if(event.key==='Enter') saveTeam(${i})">`;
    const dice_btn = `<button onclick="randomName(${i})" class="ml-1 text-sm bg-zinc-800 hover:bg-zinc-700 px-1.5 rounded" title="Nombre aleatorio">🎲</button>`;
    content = content.replace(input_str, input_str + dice_btn);
}

// 5. Bottom Actions
const bottom_actions = `<button onclick="showStats()" class="flex items-center gap-1 text-zinc-600 hover:text-zinc-300 transition-colors py-2 px-3 rounded-full hover:bg-zinc-800/40 text-xs font-semibold">
...
console.log('Modificaciones aplicadas con éxito.');
